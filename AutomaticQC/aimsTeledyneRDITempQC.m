function [data, flags, paramsLog] = aimsTeledyneRDITempQC( sample_data, data, k, type, auto )
%aimsRDITempQC Flags RDI TEMP samples while in the water as probablyBad
%
% Flags RDI TEMP samples from the data set which have a time that is during 
% in and out water time as probablyBad. The RDI TEMP sensor has a
% 
% Code based on imosInOutWaterQC.m
%
% Inputs:
...
% this test only applies to RDI workhorse/longranger/sentinel data
% TBC: new RDI Velocity, Nortek ADCP
if ~strcmp(sample_data.meta.instrument_make, 'Teledyne RDI'), return; end

% this test doesn't apply on dimensions
if ~strcmp(type, 'variables'), return; end

% this test only applies to TEMP data
if ~strcmp(sample_data.(type){k}.name, 'TEMP'), return; end
...
switch mode
    ...
    case 'timeSeries'
        ...
        probablyBadFlag  = imosQCFlag('probablyBad', qcSet, 'flag');
        ...
        if any(iGood)
            flags(iGood) = probablyBadFlag;
        else
            error(['All points failed Teledyne RDI TEMP QC set in file ' sample_data.toolbox_input_file]);
        end
